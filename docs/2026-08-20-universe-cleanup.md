...
加 `secondary_listing` 标记（**未复用 `status`**——status 会把标的逐出展示，
而 A/H 两条腿都应当可见）。仅在板块汇总的等权平均中剔除次要腿，明细热力表保留两条。

**主腿按流动性客观选定**，60 日美元成交额中位数：

| 公司 | H 股 | A 股 | 倍数 | 主腿 |
|---|---|---|---|---|
| 药明康德 | 123.5M | 1,130.6M | 9.2× | A |
| 泰格医药 | 8.2M | 148.8M | 18.1× | A |
| 康龙化成 | 16.9M | 321.9M | 19.0× | A |
| 凯莱英 | 11.7M | 257.1M | 22.0× | A |

四组一致选 A 股腿。若要改判，翻 `hc_cxo.yml` 里对应条目的 `secondary_listing` 即可。

**效果**：明细表 28 行不变；汇总成分 28 → 24（按公司去重）；双上市公司权重
7.14% → 4.17%（= 1/24，与其他成分同权）；4 家合计 28.6% → 16.7%。

**过程中修掉一个更广的静默 bug**：`upsert_members` 原本让 coverage 二轮的
`INSERT OR REPLACE` 把板块 yml 写入的标记整列抹成 NULL（2359.HK 同时在
`cmsi_coverage_hc.yml` 且带 `sector: cxo`，撞同一主键）。**同一路径也会清掉
`status`**——这次没出事只是因为 14 只退市/更名标的恰好都不在覆盖名单里。
已改为保留语义：`status` / `secondary_listing` 仅在 entry 显式给出时才写入。
