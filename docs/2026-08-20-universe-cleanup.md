...
**遗留待查（不影响本次修复）**：drillr 给 `058470.KQ` 的 2026-08-20 收盘 81,700，
yfinance 同日 69,000，差 18%。名称两源一致，非标的错配，疑为复权口径差异（韩股常见）。
KR 腿若要接第二数据源做交叉验证，须先对齐此口径。
...
**但若做市值加权或市值汇总，同一公司两条线各算一次会 ×2。**

**建议**：universe loader 增加 `primary_listing` 标记，聚合时只取一条。
