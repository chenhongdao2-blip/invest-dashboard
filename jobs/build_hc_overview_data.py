...
1. data/external/hc_index_comparison.csv
   Rebased-comparison source series, fixed anchor = 2025-08-01 (Jonah's "去年8月"):
     ...
     panel "nbi"  : ^NBI / ^IXIC                      (NBI vs Nasdaq Composite) — yfinance
...
# series_id -> (en, cn, panel, source)
SERIES_META = {
    "HSHCI.HK":  ("Hang Seng Healthcare", "恒生医疗保健", "hk",   "iFind"),
    "HSI.HK":    ("Hang Seng Index",      "恒生指数",     "hk",   "iFind"),
    "HSTECH.HK": ("Hang Seng TECH",       "恒生科技",     "hk",   "iFind"),
    "^NBI":      ("Nasdaq Biotech (NBI)", "纳斯达克生物科技", "nbi",  "yfinance"),
    "^IXIC":     ("Nasdaq Composite",     "纳斯达克综合",   "nbi",  "yfinance"),
    "^SP500-35": ("S&P 500 Health Care",  "标普500医疗保健", "sphc", "yfinance"),
    "^GSPC":     ("S&P 500",              "标普500",      "sphc", "yfinance"),
}
US_TICKERS = [s for s, m in SERIES_META.items() if m[3] == "yfinance"]
...
def build_index_comparison() -> pd.DataFrame:
    ...
    for t in US_TICKERS:
        ...
        rows.append(pd.DataFrame({"date": ser.index, "series_id": t, "close": ser.values}))
    ...
    df["panel"] = df["series_id"].map(lambda s: SERIES_META[s][2])
    df["source"] = df["series_id"].map(lambda s: SERIES_META[s][3])
