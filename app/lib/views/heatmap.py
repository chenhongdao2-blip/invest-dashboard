...
`3_Sector_Heatmap.py` + `a3_ai_heatmap.py`, 88% identical — R3 audit §7).
...
Data layer: db.market_frame(domain) — one cached cross-section (M1 USD returns +
yfinance static + fwd multiples snapshot) instead of 16 queries.
"""
...
_DOMAINS: dict[str, dict] = {
    "healthcare": {
        "cfg": "healthcare.yml",
        "key_prefix": "heatmap",
        "title_key": "heat.title",
        "cover_brand": "CMSI · SECTOR HEATMAP",
        "table_brand": "CMSI · SECTOR HEATMAP",
        "rail": section_header.RAIL_GLOBAL,
    },
    "ai": {
        "cfg": "ai.yml",
        "key_prefix": "ai_heat",
        "title_key": "ai.heat.title",
        "cover_brand": "CMSI · AI HEATMAP",
        "table_brand": "CMSI · AI SECTOR HEATMAP",
...
def render(domain: str) -> None:
    ...
    section_header.cover(i18n.t(d["title_key"]), d["cover_brand"],
    ...
    with st.sidebar:
        ui.sidebar_search(key_prefix=d["key_prefix"])
        st.divider()
    ...
    _RETS = db.returns_for(tuple(_MF.close_usd.columns), _MF.as_of, "usd", domain)
