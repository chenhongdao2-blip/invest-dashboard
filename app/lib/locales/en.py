...
STRINGS = {
    ...
    "strategy.hd.v3.method": (
        ...
        "evolution). Build rules revised 2026-07-11 and **restated back to inception**: "
        "from the v2.2 bucket-allocation scheme to a **pure qualitative-score build** "
        "(logged as a methodology-change event).\n\n"
        ...
        "2. **Pure qualitative-score selection**: the 100-name scored pool (core and "
        "buffer pools merged, no promotion line) is ranked by the *willing / able / "
        "durable to pay* total score; **top 20 enter the book**.\n"
        "3. **Pure score weighting**: weights ∝ score^1.5, higher score → higher weight; "
        "the 55/45 rate-bucket anchor is retired and the rate-premium share forms "
        "naturally (~40% at build). Risk flags (pseudo-high-yield / FCF shortfall / "
        "cycle-peak) are displayed but no longer veto selection.\n"
        "4. **Concentration limit**: single name ≤10%.\n\n"
