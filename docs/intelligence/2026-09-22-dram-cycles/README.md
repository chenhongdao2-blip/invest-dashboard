...
5. 图注（09-23 George 裁定「太多文字，尽量可视化」后压缩）：页头一行量纲 + 短图例；「数据说的话」改成四行小表（六至九轮：价格跌幅 / 美光谷底利润率 / 是否出清）；四主线各一句；第九轮五信号一行一条 + 一致预期小表（Capex / 利润率峰 / 季度线）；核心观点（09-23 第二稿）= cycle-strategist agent 按第一性原理推演的五条判断 + 证伪/证实两组 kill criteria，第九轮信号补第六条「买方融资」（推断）。事实核对改正：第七轮谷底利润率 +10.1%（原误写 +9%）、第八轮谷底 −5.1%（2022-12，原误写 +11%）、「六轮与亏损段一一对应」改为「六轮每轮都有亏损段，但 8 段亏损中 2005 / 2016 两段浅亏不在六轮内」。
...
## 核心观点五条的口径（2026-09-23）
- 数据全部来自本图已核实序列：三寡头后三轮跌幅 −25/−43/−97pp = SAW 峰谷序列；八轮跌幅中位数 −56pp = 八个 pp（−52/−60/−225/−87/−32/−25/−43/−97）排序取第四五位均值，**本图推算**，其中第一个 −52pp 从 FY1985 起点算无前峰；+24% / −17% = 80.4 减 56 / 减 97。Capex 增幅、FY27 峰、2019 谷 +10.1%、当前回撤 −5% 均见上表。
- 理论引用：中金产能周期六阶段（Capex/折旧 ≥2 倍为扩张后期标尺，MEDIUM 卖方框架）、Tvede 蛛网/加速器、周期级别服从（小级别不推翻大级别）、再融资命题（容量先于价格）。Marathon 资本周期表述不在库内，未引用。
- 「买方融资」第六信号与「天花板在买方」是理论推断（置信度中），非数据结论。
- 09-23 第三稿改为研报语体（判断句前置、术语规范、去口语），内容不变。
- 看板侧 `dram_cycles_panel.py` 守卫脚本加自适应高度：渲染后与尺寸变化时把 iframe 高设为内容高（srcdoc 与父页同源可改 `frameElement`），不再依赖固定 height；窄屏文字折行更多时也不截断。
