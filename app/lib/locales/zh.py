...
STRINGS = {
    ...
    "strategy.hd.v3.method": (
        ...
        "三代曲线完整保留（三代演进）。2026-07-11 建仓规则修订并**追溯至建仓日**："
        "由 v2.2 桶配比制改为**纯定性评分制**（登记为口径变更事件）。\n\n"
        ...
        "2. **纯定性评分选股**：100 支评分池（主池 / 缓冲池合并，不设晋升线）按"
        "「愿意分 / 分得出 / 分得久」定性总分排序，**前 20 建仓**。\n"
        "3. **纯评分定权**：评分^1.5 定权重，评分越高权重越高；不再锚定利率桶 55/45 "
        "配比，利率溢价桶占比自然形成（建仓时约 40%）。伪高息 / FCF 覆盖不足 / 周期顶等"
        "风险标注保留展示、不作一票否决。\n"
        "4. **集中度约束**：单一标的权重 ≤10%。\n\n"
