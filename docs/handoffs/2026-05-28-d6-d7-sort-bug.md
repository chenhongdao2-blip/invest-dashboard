---
status: CONSUMED
consumed_at: 2026-05-28T16:00:00+08:00
created_at: 2026-05-28T15:30:00+08:00
updated_at: 2026-05-28T16:00:00+08:00
...
consumed_summary: |
  D6 + sort-bug + D7 全部 ship。新增 app/lib/wiki.py (LLM Wiki resolver),
  app/lib/ui.render_styled_table (sort-bug-safe helper),
  app/pages/6_🔍_Ticker_Drill.py (wiki memo + price chart + multiples + live yfinance.info).
  Sort bug 在 Sector Heatmap / Healthcare / Scanner / Strategy Picks / Home 全清。
  D7 落地: .streamlit/config.toml + runtime.txt + .python-version + requirements.txt
  pinned ranges + docs/deploy/streamlit-cloud.md runbook + README Pages 表格 + 部署 section。
  AppTest harness 验证 7/7 page 无 exception (1 个 yfinance FOLD delisted warning 无关).
---
