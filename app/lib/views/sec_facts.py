...
`8_SEC_Facts.py` + `a5_ai_sec.py`, 92% identical — R3 audit §7).
...
Two `domain`s are in play and must not be conflated: `page_domain` selects the
US pool this nav entry shows, while `row_domain` is the selected ticker's own
domain and picks its KPI overlay. The originals could call both `domain` because
each file hard-coded the first one.
"""
...
_DOMAINS: dict[str, dict] = {
    "healthcare": {
        "key_prefix": "sec",
        "title_key": "sec.title",
        "caption_key": "sec.caption",
        "brand": "CMSI · SEC FACTS",
        "rail": section_header.RAIL_US,
    },
    "ai": {
        "key_prefix": "ai_sec",
        "title_key": "ai.sec.title",
        "caption_key": "ai.sec.caption",
        "brand": "CMSI · AI SEC FACTS",
        "rail": section_header.RAIL_AI,
    },
}


def render(page_domain: str) -> None:
    d = _DOMAINS[page_domain]
    kp = d["key_prefix"]          # widget-key prefix; NOT `k` — `for k in kpis` below

    # ── sidebar + lang ──
    i18n.init_lang()
    with st.sidebar:
        ui.sidebar_search(key_prefix=kp)
    i18n.render_lang_toggle()
    section_header.cover(i18n.t(d["title_key"]), d["brand"],
                         rail=d["rail"], prefer_cn=i18n.get_lang() == "zh")
    st.caption(i18n.t(d["caption_key"]))

    pool = sf.us_pool()
    pool = pool[pool["domain"] == page_domain].copy()
    us_tickers = pool["ticker"].tolist()
    prefer_cn = i18n.get_lang() == "zh"
    name_map = db.ticker_to_name(prefer_cn=prefer_cn)

    # Empty pool (a domain whose US names have not entered the SEC fetch pool yet):
    # same prompt + stop the ticker check below would produce, just one step earlier.
    if not us_tickers:
    ...
    # the SELECTED TICKER's own domain (drives which KPI overlay shows) —
    # distinct from page_domain, which chose the pool above
    row_domain = pool.set_index("ticker")["domain"].get(ticker)

    # ══════════════════════════════════════════════════════════════════════════
    # ① KPI cards (domain-aware) + auditable source trace
    # ══════════════════════════════════════════════════════════════════════════
    theme.section_header(i18n.t("sec.section.kpi"))
    kpis = sf.kpis_for_domain(row_domain)
    ...
    sec_statements.statements_section(
        ticker, status.get("taxonomy_primary") or "us-gaap", prefer_cn, freq=period, domain=row_domain
    )
    ...
    with st.expander(_raw_title, expanded=False):
        if all_df.empty:
            ...
            q = fc1.text_input(i18n.t("sec.browser.search"), key=f"{kp}_browse_q")
            form_opts = ["(all)"] + sorted(x for x in all_df["form"].dropna().unique())
            tax_opts = ["(all)"] + sorted(all_df["taxonomy"].dropna().unique())
            sel_form = fc2.selectbox(i18n.t("sec.browser.form"), form_opts, key=f"{kp}_browse_form")
            sel_tax = fc3.selectbox(i18n.t("sec.browser.taxonomy"), tax_opts, key=f"{kp}_browse_tax")
    ...
    comp_tickers = st.multiselect(
        ...
        key=f"{kp}_comp_tickers",
    )
    all_kpi_keys = [k["kpi_key"] for k in sf.kpis_for_domain(row_domain)]
    kpi_labels = {k["kpi_key"]: k[label_field] for k in sf.kpis_for_domain(row_domain)}
    ...
    if comp_tickers and comp_kpis:
        ...
        unit_by_label = {k[label_field]: (k["unit_hint"] or "USD") for k in sf.kpis_for_domain(row_domain)}
        prev = comp.copy()
