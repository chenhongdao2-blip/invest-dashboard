export default function(component) {
  ...
  const visible = () => {
    ...
    const value=r=>state.view==='person'?key==='s3'?(() => {const parts=[r.s3_realized_cny,r.s3_exposure_cny].filter(Number.isFinite);return parts.length?parts.reduce((x,y)=>x+y,0):null;})():r[key+'_cny']:(key==='s12'?r.s12_pct:key==='ratio'?r.total_pct:key==='policy'?r.policy_return:key==='marketcap'?r.market_cap_cny:r.total_cny);
    filtered.sort((a,b)=>{
      const va = value(a);
      const vb = value(b);
  ...
  function render(){
    ...
    const headings=state.view==='company'?['公司 / 代码','合计（可算）','总市值·人民币','S1+S2 / 市值','S1+S2+S3 / 市值','政策以来']:['人物 / 公司','合计（可算）','S1 存续','S2 装入','S3 已发生 / 敞口'];
    const sortKeys=state.view==='company'?['name','total','marketcap','s12','ratio','policy']:['name','combined_total','s1','s2','s3'];
