...
STRINGS = {
    ...
    "strategy.ipo.tab.intro": (
        "**港股打新策略后测** — 用 CMSI 棱镜六因子模型对近期港股新股打分分档，回看评分与首日表现的"
        "关系。样本覆盖近期港股新股，已上市标的的中位首日收益、方向命中率等统计见下方 KPI 卡"
        "（随样本更新）。核心读数:**评分擅长判方向（打不打），不擅长测涨幅（涨多少）**——评分与"
        "首日涨幅仅弱相关（见散点右上角 Spearman ρ）。下方散点看评分 × 首日，小图看每只盘中路径，"
        "双榜可按评分或首日涨幅排序。"
    ),
    # KPI
    ...
    "strategy.ipo.kpi.med": "已上市中位首日",
    "strategy.ipo.kpi.med_delta": "区间 {lo:+.0f}% ~ {hi:+.0f}%",
    "strategy.ipo.kpi.hitrate": "首日收涨率",
    "strategy.ipo.kpi.hitrate_delta": "{up}/{n} 只首日收涨",
    "strategy.ipo.kpi.note": (
        "推荐档及以上（评分 ≥6.0）首日收涨 **{rec_up}/{rec_n}**；全样本收涨 {up}/{n}"
        "（含谨慎 / 不申购档）。首日涨幅受曦智 +384%、深演 +266%、商米 +241% 等右尾极值拉动，"
        "**中位 {median:+.1f}% 更代表中枢收益**。本页为内部研究回测，不构成任何申购建议。"
    ),
    # 散点图
    ...
    # 方法论
    "strategy.ipo.method_expander": "打新策略方法论 · 六因子评分",
    "strategy.ipo.method": (
        "**港股 IPO 六因子评分体系（CMSI 棱镜打新卡）**\n\n"
        "本策略以六个维度对每只新股加权打分（满分 10 分），据此分四档给出申购建议。"
        "**与生物科技催化剂策略的临床 / FDA / 财报 / 治理维度无关**——打新评估的是"
        "「上市定价博弈与首日供需」，不是企业长期基本面。\n\n"
        "| # | 因子 | 评估内容 |\n"
        "|---|---|---|\n"
        "| ① | 流通盘稀缺度 | 自由流通市值 / 公众持股比例；货少则首日筹码博弈更激烈 |\n"
        "| ② | 基石阵容 / 机构背书 | 基石投资者质量与认购占比，知名长线 / 产业资本进场 |\n"
        "| ③ | 行业稀缺性 / 板块景气 | 标的是否稀缺赛道（如光子 AI、AI 制药）、当期板块情绪 |\n"
        "| ④ | 公开发售认购倍数 | 散户超额认购倍数，反映市场热度与回拨后供需 |\n"
        "| ⑤ | 估值合理性 | 招股定价相对可比公司 / 一级估值的折溢价空间 |\n"
        "| ⑥ | 公司基本面与财务质量 | 收入增速、毛利、现金与盈利路径等底层质地 |\n\n"
        "**四档阈值：**\n"
        "- 评分 ≥ 7.5 → **重点申购+**\n"
        "- 6.0 – 7.4 → **推荐申购**\n"
        "- 5.0 – 5.9 → **谨慎申购**\n"
        "- < 5.0 → **不申购**\n\n"
        "**后测读数（n=17 已上市）：** 评分 ≥ 6.0（推荐及以上）的新股首日收涨命中率 "
        "**90.9%（10/11）**，全样本首日收涨率 88.2%。但需注意：评分与首日**涨幅大小**仅弱相关"
        "（Spearman ρ=0.13，不显著）——**评分是「要不要打」的过滤器，不是「能涨多少」的预测器**，"
        "首日涨幅更多由稀缺性、认购热度与开盘竞价博弈驱动。"
    ),
    "strategy.ipo.caveat": (
