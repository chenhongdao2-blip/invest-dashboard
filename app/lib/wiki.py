...
# AI company wiki — nested under layer subdirectories (L1-设备与材料/ etc.).
# Only available on George's Mac.
_INTERNAL_AI_WIKI_ROOT = Path.home() / "Documents" / "LLM Wiki" / "Wiki" / "AI" / "companies"
...
def _wiki_roots() -> list[tuple[Path, bool]]:
    ...
    if _INTERNAL_AI_WIKI_ROOT.exists():
        roots.append((_INTERNAL_AI_WIKI_ROOT, False))
...
def _resolve_path(ticker: str) -> tuple[Path, bool] | None:
    ...
    Uses rglob so that nested layer subdirectories (e.g. AI/companies/L2-芯片设计/)
    are searched recursively. rglob is safe on flat directories too.
    """
