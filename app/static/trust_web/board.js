export default function(component) {
  ...
  const defaults = {view:'company', market:'HK', query:'', sort:'total', dir:-1, minS12:'', minTotal:'', status:'all', grade:'strong_medium', advanced:false, scroll:0, restoreScroll:false};
  ...
  const visible = () => {
    ...
    const filtered = rows.filter(r => {
      if(state.view==='company') {
        if(r.market!==state.market || (state.status!=='all'&&r.status!==state.status)) return false;
        ...
        if(state.market!=='all' && !r.company_codes.includes(state.market)) return false;
      }
  ...
  function render(){
    ...
    root.querySelector('[data-market]').onchange=event=>{state.market=event.target.value;persist();render();};
    ...
    root.querySelector('[data-advanced]').onclick=()=>{state.advanced=!state.advanced;persist();render();};
